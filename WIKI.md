...
- 적합도(매칭품질): analyzeJobFit(jobText) — 내 용어(parseSkills 이름 + 모든 카드 tags/techStack + position 분해) ∪ COMMON_JD_KEYWORDS(코드내 약80개 일반 채용키워드 사전)를 vocab으로, 공고본문(소문자)에 `includes`로 존재하는 term만 추려 mine 포함=matched·미포함=missing, score=matched/(matched+missing)%. fitPanelHtml을 renderJobMatches 상단에 합쳐 표시(% 막대+칩). **빠진키워드 검출은 사전(COMMON_JD_KEYWORDS)이 좌우** — 실공고로 노이즈/누락 키워드 튜닝 필요. 클라이언트 전용(분석 AI 호출과 무관).
- 지원이력 그룹핑: goHistory 항목에 jobKey=jobKeyOf(title)(소문자+비단어 제거 40자) 저장. renderHistory가 jobKey로 묶어 hist-group 카드(분석→자소서→이력서 순서 ORDER). 구이력(jobKey 없음)도 render시 jobKeyOf(title)로 자동 그룹. 제목이 다르면 같은 공고여도 분리됨(한계).
- Word(.doc) 내보내기: downloadDoc(filename,text) — 텍스트를 office 네임스페이스 HTML(<html xmlns:o/w>)로 감싸고 줄바꿈→<br>, Blob(['﻿', html], {type:'application/msword'})로 .doc 저장. 라이브러리 없이 Word/한글에서 열림. downloadGroup(key,fmt)는 한 공고 묶음을 구분선과 함께 한 파일로(txt/doc).
- PDF 한글 깨짐(CID/ToUnicode 없는 폰트) 대응: 일부 PDF는 pdf.js `it.str`이 CJK확장A/기호/PUA로 깨져 나옴(예 '㢨䚌⫤'). 이때 AI 분석은 안 깨지는 영어토큰·숫자만 보고 한글(프로젝트명·학력 등)을 지어냄(할루시네이션). 감지=`koreanTextLooksBroken(text)`(index.html): 정상 한글음절(가-힣) 수 대비 garble 글자(`[⺀-㏿㐀-䶿豈-﫿+PUA]` = U+2E80~33FF·3400~4DBF·F900~FAFF·E000~F8FF)가 20개 이상이고 hangul보다 많으면 깨짐. 깨지면 `pdfToOcrText(file)`이 fileToBase64 후 `/api/ocr-job`에 `mimeType:'application/pdf'`로 보내 재추출(Upstage document-digitization은 PDF 직접입력 지원, ocr-job.js 무수정). 정상/영어전용 이력서는 garble≈0이라 오탐 없음(node로 검증). OCR도 깨지거나 빈값이면 docx·붙여넣기 안내로 폴백.
- analyze-profile 충실도 규칙: 고유명사(프로젝트/회사/학교/과정/자격증명)·날짜·수치·표현은 원문 그대로, 요약·일반화·학위상향(고등학교→학사) 금지, 사실정보 추정 금지(추정은 position/experience/strengths만+uncertain 표기). skills.detail은 원문 활용역량/정량정보만('5년 이상 사용' 등 없는 근속 지어내기 금지). projectCards.title에 기술명(techStack) 넣기 금지.
- careerCards 파이프라인(주의): 프론트(index.html:1547)는 `analysis.careerCards`를 회사명+기간 키로 중복제거 병합. 서버 `parseProfileJson` 반환 객체에 `careerCards: sanitizeCareerCards(parsed.careerCards)`를 **반드시 포함**해야 AI 경력카드가 채워짐(누락되면 프론트가 항상 빈배열로 처리). 필드=company/period/role/summary/contributions[]/outcomes[], company 없으면 버림.
